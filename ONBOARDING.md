...
2. **عند الاستدعاء، الـ skill بتسألك سؤالاً واحداً منظّماً أولاً** — كيف بدّك تعطيها الـ API؟ اختر واحداً:
   - **مجموعة Postman/Insomnia** (`.json` مُصدّرة) — تشتقّ منها الـ endpoints والأمثلة.
   - **ملف OpenAPI/Swagger** (مسار أو رابط) — تولّد منه baseline تلقائياً عبر `tools/gen_baseline.py`.
   - **رابط staging فقط** — تكتشف السطح وتوضّح ما يحتاج spec.
   - **baseline محفوظ** — لإعادة اختبار عقد سابق.

   ومعها: **رابط staging مصرّح به + المصادقة (الـ token من متغيّر بيئة فقط) + هل الكتابة مسموحة**. الافتراضي: قراءة-فقط على أصغر نطاق.
